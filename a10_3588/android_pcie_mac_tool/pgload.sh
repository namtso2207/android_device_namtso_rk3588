...
if [ "$check" != "" ]; then
        echo "rmmod r8169"
        /system/bin/rmmod r8169
fi

check=`lsmod | grep r8168`
if [ "$check" != "" ]; then
        echo "rmmod r8168"
        /system/bin/rmmod r8168
fi

check=`lsmod | grep r8125`
if [ "$check" != "" ]; then
        echo "rmmod r8125"
        /system/bin/rmmod r8125
fi

check=`lsmod | grep r8101`
if [ "$check" != "" ]; then
        echo "rmmod r8101"
        /system/bin/rmmod r8101
fi
...
if [ "$check" != "" ]; then
        ...
        if test -e ./$module.ko ; then
                ...
                /system/bin/insmod /data/android_pcie_mac_tool/$module.ko $* || exit 1
        else
                echo "make clean all"
                make clean all 1>/dev/null
                if test -e /data/android_pcie_mac_tool/$module.ko ; then
                        echo "insmod $module.ko $*"
                        /system/bin/insmod /data/android_pcie_mac_tool/$module.ko $* || exit 1
                else
